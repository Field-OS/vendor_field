...
func init() {
	android.RegisterModuleType("field_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = fieldExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = fieldExpandVariables(ctx, dep_file)
	...
	cmd := fieldExpandVariables(ctx, String(g.properties.Cmd))
